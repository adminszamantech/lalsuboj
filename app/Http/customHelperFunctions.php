...
function bnHeaderToCategory()
{
    return \App\Models\BnCategory::where('cat_type', 1)->where('top_menu', 1)->where('status', 1)->where('deletable', 1)->orderBy('cat_position')->take(13)->get();
}
...
function postURL($CatSlug = null, $ContentID = null)
{
    $sURL = url("/details/" .$CatSlug   . "/" . $ContentID);
    return $sURL;
}
function printPostURL($CatSlug = null, $ContentID = null)
{
    $sURL = url("/print-details/" .$CatSlug   . "/" . $ContentID);
    return $sURL;
}
...
{
    $sURL = url('/photos/' . $catSlug . "/" . ($subCatSlug != null ? $subCatSlug : 'album') . "/". $albumID);
    return $sURL;
}

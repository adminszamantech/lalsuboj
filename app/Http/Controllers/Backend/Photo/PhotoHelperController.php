...
{
    public static function getPositionContent($positionId = null, $catId = null)
    {
        $contents = collect();
        ...
        if ($position && $position->content_ids) {
            ...
            if (!empty($aContentIDs)) {  // Ensure $aContentIDs is not empty
                ...
                $contents = PhotoAlbum::with('category')
                    ->whereIn('album_id', $aContentIDs)
                    ->where('deletable', 1)
                    ->orderByRaw("FIELD(album_id, $sContentIDs)")
                    ->get();
            }
        }

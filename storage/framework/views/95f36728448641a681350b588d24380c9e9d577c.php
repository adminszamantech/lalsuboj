...
<!-- Content Header (Page header) -->
    <section class="content-header">
        <ol class="breadcrumb" style="display: inline-block; width: 500px">
            <li><a href="<?php echo e(route('bn-content-position.index')); ?>">Photo Album Position List</a></li>
                ...
                <div class="box box-info">
                    <div class="box-body">
                        ...
                        <div class="form-inline">
                            ...
                            <input type="hidden" name="position" value="<?php echo e($album_position->position_id); ?>">
                            ...
                            <div class="form-group">
                                ...
                                <input type="hidden" id="positionId" value="<?php echo e($album_position->position_id); ?>">
                            ...
                            <div class="col-sm-1 text-right position-number">
                                <ul id="serial">
                                    <?php if(!empty($allAlbums)): ?>
                                        <?php for($i=1; $i <= count($allAlbums); $i++): ?>
                                            <li><?php echo e($i); ?></li>
                                        <?php endfor; ?>
                                    <?php endif; ?>
                                </ul>
                                ...
                                <ul id="sortable" style="padding: 0">
                                    <?php if(!empty($allAlbums)): ?>
                                        <?php $__currentLoopData = $allAlbums; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $album): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <li class="item" id="item-<?php echo e($album->album_id); ?>">
                                                <?php echo e($album->album_name); ?>

                                                <span class="badge"><?php echo e($album->album_id); ?></span>
                                                <button id="removePosition" class="btn btn-xs btn-warning pull-right" onclick="removeLi(<?php echo e($album->album_id); ?>)">X</button>
                                            </li>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    <?php endif; ?>
                                    <input type="hidden" value="<?php echo e($album_position->position_id); ?>" id="positionId">
                                    <button type="button" onclick="saveData(<?php echo e($album_position->position_id); ?>)" class="btn btn-primary">Save</button>
                                </ul>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function saveData(posId) {
            ...
            if(data.length == 0){
                ...
                var id = posId;
                $.post('<?php echo e(url("backend/photo-album-position/change")); ?>', {'_token': '<?php echo e(csrf_token()); ?>', "data": data, "settings": settings, 'id': id}, function(data){
                    alert("The position has been changed!");
                    window.location.href = '<?php echo e(url("backend/photo-album-position/change")); ?>/'+data.position_id;
                });
            }
        ...
        $(function() {
            $("#newsHeading").autocomplete({ // For news title autocomplete
                source: function(request, response) {
                    $.get("<?php echo e(url('backend/photo-album-position/keyword')); ?>", { posId: $('#positionId').val(), term: $('#newsHeading').val() }, response);
                },
                minLength: 1,
        ...
        $(function() {
            ...
            var positionId = '<?php echo e($album_position->position_id); ?>';
            ...
            $('#positionName').change(function() {
                ...
                //alert(position);
                $.get('<?php echo e(url("backend/photo-album-populate-position")); ?>', {'position': position}, function (data) {
                    $('#serial').empty();
                    var no = 1;

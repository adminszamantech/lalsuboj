...
@section('title', request()->get('q') . ' । অনুসন্ধান । নতুনদেশ২৪.কম')
...
{{--                    <div class="border-b border-custom-bc dark:border-gray-600 last:border-0 py-4 even:border-r-2">--}}
                        <a href="{{ postURL($content->category->cat_slug, $content->content_id) }}" class="grid grid-cols-12 py-4 gap-4 group border-b border-custom-bc dark:border-gray-600 last:border-b-0">
                            <div class="overflow-hidden col-span-4">
                                <img class="w-full h-full group-hover:scale-110 duration-500" src="{{ $content->img_bg_path ? asset(config('appconfig.contentImagePath').$content->img_bg_path) : asset(config('appconfig.commonImagePath').Cache::get('bnSiteSettings')->og_image) }}" alt="">
                            </div>
                            <div class="col-span-8">
                                <div class="flex flex-col gap-2">
                                    <h2 class="text-[14px] md:text-[18px] m-0 text-base-color font-semibold pr-2 group-hover:text-base-color-hover dark:text-slate-200">{{ $content->content_heading }}</h2>
{{--                                    <p class="dark:text-slate-300"><i class="fa fa-clock-o"></i> {{fFormatDateEn2Bn(date('d F, Y')) }}</p>--}}
                                </div>
                            </div>
                        </a>
{{--                    </div>--}}
    ...
    <script>
        ...
        function loadMoreSearchContent(){
            ...
            axios.post('{{ route('search.more.post') }}', {
                _token: "{{ csrf_token() }}",
                offset: offset,
                q: "{{ $keyword }}"
            }).then(response => {

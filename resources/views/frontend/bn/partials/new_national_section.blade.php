    ...
    <div class="col-span-12 md:col-span-12 relative md:after:content-[''] md:after:absolute after:top-0 after:-right-4 after:w-[1px] after:h-full md:after:border-r after:dark:border-gray-600 after:border-custom-bc items-center  dark:border-[#fff2]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 md:gap-8 py-4 border-b border-custom-bc dark:border-gray-600">
            ...
            <div class="col-span-12 md:col-span-3 mt-6 md:mt-0 ">
                <!-- Second Lead Post --->
                @if($nationalContents[0])
                    <a href="{{ postURL($nationalContents[0]->category->cat_slug, $nationalContents[0]->content_id) }}" class="flex flex-col gap-2 hover:no-underline focus:no-underline group group relative">
                        <div class="overflow-hidden">
                            <img class="w-full group-hover:scale-110 duration-500" src="{{ $nationalContents[0]->img_bg_path ? asset(config('appconfig.contentImagePath').$nationalContents[0]->img_bg_path) : asset(config('appconfig.commonImagePath').Cache::get('bnSiteSettings')->og_image) }}" alt="">
                        </div>
                        <div>
                            <h2 class="post-title m-0 py-2 text-base-color group-hover:text-base-color-hover dark:text-slate-300"><strong>{{ $nationalContents[0]->content_heading }}</strong></h2>
                            <p class="lead-short-text text-black dark:text-gray-400">{!! fGetWord(fFormatString($nationalContents[0]->content_details), 8) !!}</p>
                        </div>
                    </a>
            @endif
            <!--/ Second Lead Post --->
        ...
        <div class="grid grid-cols-2 gap-2 md:grid-cols-4 md:gap-8 pt-4 pb-4 md:border-b border-b-0 border-custom-bc dark:border-gray-600">
            ...
            @foreach($nationalBtContents as $ntBottomCont)
                @if($ntBottomCont)

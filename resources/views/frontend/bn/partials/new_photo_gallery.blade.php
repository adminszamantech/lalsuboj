<div class="flex flex-col py-4">
    ...
    <div class="grid grid-cols-12 gap-4 py-4 border-b border-custom-bc dark:border-gray-600">
        ...
        <div class="col-span-12 md:col-span-6">
            <a href="{{ $leadAlbumURL }}" class="relative">
                <img class="h-full" src="{{ asset(config('appconfig.photoAlbumImagePath') . ($leadAlbum->feature_image['img'])) }}"
                     alt="{{ $leadAlbum->album_name }}">
                <div class="absolute bottom-0 left-0 text-white px-4 bg-black py-2 w-full">
                    <h2>{{ $leadAlbum->album_name }}</h2>
                </div>
            </a>
        </div>
        <div class="col-span-12 md:col-span-6 h-full">
            <div class="grid grid-cols-2 gap-4 h-full">
                @foreach($photoAlbums->take(4) as $album)
                    @php($albumURL = fAlbumURL($album->album_id, $album->category->cat_slug))
                    <a href="{{ $albumURL }}" class="relative group overflow-hidden h-full">
                        <img class="h-full" src="{{ asset(config('appconfig.photoAlbumImagePath') . ($album->feature_image['img'])) }}" alt="{{ $album->album_name }}">

...
@section('title', $subcategory->subcat_name_bn.' । '.Cache::get('bnSiteSettings')->site_name)
...
@section('mainContent')
            ...
            <div class="border-b-[1px] border-custom-dbc">
                <div class="mb-3">
                    <a class="block w-fit" href="#">
                        <h1 class="text-xl md:text-2xl dark:text-slate-200">{{ $subcategory->subcat_name_bn }}</h1>
                    </a>
                </div>
                ...
                @if(count($subCategories) > 0)
                    <div class="mb-3 flex flex-wrap flex-row gap-4">
                        @foreach($subCategories as $subCategory)
                            <a class="text-lg hover:text-base-color-hover relative last:after:hidden after:content-[''] after:absolute after:top-[6px] after:-right-2 after:h-[65%] after:border-r-2 after:border-custom-bc after:dark:border-gray-600 dark:text-slate-300" href="{{ url($category->cat_slug.'/'.$subCategory->subcat_slug) }}">{{ $subCategory->subcat_name_bn }}</a>
                        @endforeach
                    </div>
                @endif
            </div>
                ...
                <div class="col-span-12 lg:col-span-8 xl:col-span-9 relative after:content-[''] after:absolute after:top-0 after:-right-3 after:w-[1px] after:h-full after:border-r after:dark:border-gray-600 after:border-custom-bc">
                        ...
                        <div class="category_head_content border-b border-custom-bc dark:border-gray-600 pb-4">
                            <div class="grid grid-cols-12 gap-6">
                                <div class="col-span-12 md:col-span-6 relative after:content-[''] after:absolute after:top-0 after:-right-3 after:w-[1px] after:h-full after:border-r after:border-custom-bc after:dark:border-gray-600">
                                    @if(@count($contents) > 0)
                                        <a href="{{ postURL($contents[0]->category->cat_slug, $contents[0]->content_id) }}" class="flex flex-col gap-2 hover:no-underline focus:no-underline group">
                                            <div class=" overflow-hidden">
                                                <img class="w-full group-hover:scale-110 duration-500" src="{{ $contents[0]->img_bg_path ? asset(config('appconfig.contentImagePath').$contents[0]->img_bg_path) : asset(config('appconfig.commonImagePath').Cache::get('bnSiteSettings')->og_image) }}" alt="{{ $contents[0]->content_heading }}">
                                            </div>
                                            <div class="flex flex-col gap-3">
                                                <h2 class="category-heading-text text-base-color group-hover:text-base-color-hover dark:text-slate-300 leading-[1.68rem]">
                                                    {{ $contents[0]->content_heading }}
                                                </h2>
                                                <p class="lead-short-text text-[#555555] dark:text-gray-400">{!! fGetWord(fFormatString($contents[0]->content_details), 20) !!}</p>
                                            </div>
                                        </a>
                                    @endif
                                ...
                                <div class="col-span-12 md:col-span-6 pt-4 md:pt-0 border-t border-custom-bc dark:border-gray-600 md:border-none">
                                    <div class="flex flex-col gap-8">
                                        @foreach($contents->slice(1, 3) as $feature_content)
                                            <a href="{{ postURL($feature_content->category->cat_slug, $feature_content->content_id) }}" class="grid grid-cols-12 gap-2 hover:no-underline focus:no-underline group relative after:content-[''] after:absolute after:-bottom-4 after:left-0 after:w-full after:h-[1px] after:border-b after:border-custom-bc last:after:border-0 after:dark:border-gray-600">
                                                <div class="col-span-7 md:col-span-6">
                                                    <h2 class="text-[19px] m-0 py-2 text-base-color group-hover:text-base-color-hover dark:text-slate-300 leading-[1.68rem]">{{ $feature_content->content_heading }}</h2>
                                                </div>
                                                <div class="col-span-5 md:col-span-6 overflow-hidden">
                                                    <img class="w-full group-hover:scale-110 duration-500" src="{{ asset(config('appconfig.contentImagePath').$feature_content->img_bg_path) }}" alt="{{ $feature_content->content_heading }}">
                                                </div>
                                            </a>
                                        @endforeach
                                    </div>
                    ...
                    <div class="category_loop_content py-4">
                        ...
                        @foreach($contents->slice(4) as $feature_bottom_content)
                            <!-- Loop Content -->
                                <a href="{{ postURL($feature_bottom_content->category->cat_slug, $feature_bottom_content->content_id) }}" class="col-span-12 md:col-span-6 relative group after:content-[''] after:absolute after:top-0 after:-right-3 after:w-[1px] after:h-full after:border-r last:after:border-0 after:border-custom-bc after:dark:border-gray-600">
                                    <div class="grid grid-cols-12 gap-2">
                                        <div class="col-span-7">
                                            <h2 class="text-[19px] text-base-color dark:text-slate-300 group-hover:text-base-color-hover leading-[1.68rem]">{{ $feature_bottom_content->content_heading }}</h2>
                                        </div>
                                        <div class="col-span-5 overflow-hidden">
                                            <img class="w-full group-hover:scale-110 duration-500" src="{{ $feature_bottom_content->img_bg_path ? asset(config('appconfig.contentImagePath').$feature_bottom_content->img_bg_path) : asset(config('appconfig.commonImagePath').Cache::get('bnSiteSettings')->og_image) }}" alt="{{ $feature_bottom_content->content_heading }}">
                                        </div>
                                    </div>
    ...
    <script>
        ...
        let subcategoryId = "{{ $subcategory->subcat_id }}"; // Category ID
        function loadMoreContent(){
            ...
            axios.post('{{ route('load.more.subcategory.posts') }}', {
                _token: "{{ csrf_token() }}",

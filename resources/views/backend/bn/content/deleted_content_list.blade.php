            ...
            <div class="box-header with-border">
              ...
              <div class="pull-right">
                ...
                @if($contents->count() > 0 && $contents[0]->deletable == 1)
                  <a href="{{ url('admin/backend/deleted-bn-contents') }}" class="btn btn-info btn-sm">Deleted List</a>
                @else
                  <a href="{{ url('admin/backend/bn-contents') }}" class="btn btn-info btn-sm">Content List</a>
                @endif
              </div>
            </div>
            <!-- /.box-header -->
              ...
              <tbody>
                @foreach($contents as $no => $content)
                  <tr>
                    ...
                    <td>
                        <a href="{{ url('admin/backend/enable-bn-content/'.$content->content_id) }}" class="btn btn-warning btn-xs" onclick="return confirm('Are you sure want to enable the content?')">Enable</a>
                    </td>
                  </tr>
                @endforeach
              </tbody>
